        ...
        if service_id and city:
            try:
                ...
                if can_travel.lower() == 'true':
                    ...
                    photographer_query = photographer_w_service.filter(
                        Photographer.city == city).order_by(Photographer.name).all()
            except Exception as e:
                print(e)
                abort(422)
            ...
            for photographer in photographer_query:
                ...
                photographer_info['photos'] = [photo.format()
                                               for photo in photo_query]
                photographer_info['price'] = price_query.format_by_service(
                    service_id)
